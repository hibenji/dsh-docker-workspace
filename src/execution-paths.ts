 ...
 */
export function mapExecutableForContainer(
  command: string,
  sourceToContainer: (path: string) => string | undefined,
  resolvedContainerExecutables: ReadonlySet<string>,
  hostPathExists: (path: string) => boolean = existsSync,
): string {
  if (!isAbsolute(command)) return command
  const workspacePath = sourceToContainer(command)
  if (workspacePath !== undefined) return workspacePath
  if (resolvedContainerExecutables.has(command)) return command
  if (hostPathExists(command)) {
    const base = basename(command).replace(/\.exe$/i, '')
    if (/^(rg|ripgrep)(?:[-_.].*)?$/i.test(base)) return 'rg'
    return base
  }
...
export function mapArgumentForContainer(
  argument: string,
  sourceToContainer: (path: string) => string | undefined,
): string {

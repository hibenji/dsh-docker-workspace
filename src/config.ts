...
export interface DockerWorkspaceConfig {
  ...
  network?: string
  removeContainerOnAgentDispose?: boolean
  keepWorkspace?: boolean
  copyNodeModules?: boolean
}

export interface ResolvedDockerWorkspaceConfig {
  presetId: string
  image: string
  autoBuildImage: boolean
  workspaceRoot: string
...
export function resolveConfig(config: DockerWorkspaceConfig = {}): ResolvedDockerWorkspaceConfig {
  ...
  if (image.length === 0) throw new Error('dsh-docker-workspace: image must be non-empty')
  if (!isAbsolute(workspaceRoot)) throw new Error(`dsh-docker-workspace: workspaceRoot must be absolute: ${workspaceRoot}`)
  ...
  if (network.length === 0) throw new Error('dsh-docker-workspace: network must be non-empty')

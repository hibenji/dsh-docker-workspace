...
class SpillCapture {
  ...
  private bytes = 0
  private intact = true
  constructor(readonly hostPath: string, readonly containerPath: string, readonly maxBytes: number) {
    mkdirSync(dirname(hostPath), { recursive: true })
    this.fd = openSync(hostPath, 'w', 0o600)
  }

  append(chunk: Buffer): void {
    if (!this.intact || this.fd === undefined) return
    this.bytes += chunk.length
    if (this.bytes > this.maxBytes) {
      this.intact = false
      closeSync(this.fd)
      this.fd = undefined
      try { unlinkSync(this.hostPath) } catch {}
      return
    }
...
class DockerProcessHandle implements SubprocessHandle {
  ...
  private terminatedBy: NodeJS.Signals | null = null
  private terminating = false
  private closed = false

  constructor(
    child: ChildProcess,
    private readonly state: DockerSessionWorkspace,
    private readonly pidFile: string,
    private readonly graceMs: number,
    stdoutCollector: CollectorState | undefined,
    stderrCollector: CollectorState | undefined,
    signal?: AbortSignal,
  ) {
    this.child = child
    ...
    if (stdoutCollector !== undefined) child.stdout?.on('data', chunk => {
      const data = Buffer.from(chunk)
      stdoutCollector.reader.append(data)
      stdoutCollector.spill?.append(data)
    })
    if (stderrCollector !== undefined) child.stderr?.on('data', chunk => {
      const data = Buffer.from(chunk)
  ...
  async waitForExit(signal?: AbortSignal): Promise<boolean> {
    for (;;) {
      ...
      if (!alive && this.closed) return true
...
export class DockerSubprocessRuntime extends SubprocessRuntime {
  ...
  private readonly live = new Set<DockerProcessHandle>()
  private readonly resolvedContainerExecutables = new Set<string>()
  ...
  async resolveExecutable(command: string, env?: Readonly<Record<string, string>>, signal?: AbortSignal): Promise<string> {
    if (command.length === 0) throw new Error('docker-subprocess: executable must be non-empty')
  ...
  spawn(spec: SubprocessSpawnSpec): SubprocessHandle {
    ...
    if (program === undefined || program.length === 0) throw new Error('docker-subprocess: argv must contain a program')
    ...
    const envCommand: string[] = ['env']
    for (const [key, value] of environment) {
      if (value === undefined) envCommand.push('-u', key)
      else envCommand.push(`${key}=${value}`)
    }
  ...
  private mapCwd(state: DockerSessionWorkspace, cwd: string): string {
    if (cwd === state.containerWorkspace || cwd.startsWith(`${state.containerWorkspace}/`)) return posix.normalize(cwd)
    const mapped = this.ctx.dockerWorkspace.sourceToContainer(state, resolve(cwd))
    if (mapped === undefined) throw new Error(`docker-subprocess: cwd is outside isolated workspace: ${cwd}`)
    return mapped
  }

  private mapExecutable(state: DockerSessionWorkspace, command: string): string {
    return mapExecutableForContainer(
      command,
      path => this.ctx.dockerWorkspace.sourceToContainer(state, path),
      this.resolvedContainerExecutables,
    )
  }

  private mapArgument(state: DockerSessionWorkspace, arg: string): string {
    return mapArgumentForContainer(
      arg,
      path => this.ctx.dockerWorkspace.sourceToContainer(state, path),
    )
  }
...
function isContainerAbsolute(path: string): boolean {
  return path.startsWith('/')
}

function nodeOutputMode(mode: SubprocessOutputMode): 'pipe' | 'inherit' {
  return mode === 'inherit' ? 'inherit' : 'pipe'
}

function collectState(mode: SubprocessOutputMode, state: DockerSessionWorkspace, token: string, stream: 'stdout' | 'stderr'): CollectorState | undefined {
...
function createSpill(mode: SubprocessCollect, state: DockerSessionWorkspace, token: string, stream: string): SpillCapture | undefined {
  if (mode.spill === undefined) return undefined
...
async function remoteSignal(container: string, pidFile: string, signal: 'TERM' | 'KILL'): Promise<void> {
  const script = [
...
async function remoteGroupAlive(container: string, pidFile: string): Promise<boolean> {

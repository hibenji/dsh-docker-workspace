...
export function safeSessionSlug(sessionId) {
...
export function isInsideHost(parent, child) {
    const rel = relative(resolve(parent), resolve(child));
    return rel === '' || (rel !== '..' && !rel.startsWith(`..${sep}`) && !isAbsolute(rel));
}
export function sourcePathToContainer(sourceRoot, sourcePath, containerRoot = '/workspace') {
    const absolute = resolve(sourcePath);
    if (!isInsideHost(sourceRoot, absolute))
        return undefined;
    const rel = relative(resolve(sourceRoot), absolute).split(sep).join('/');
    return rel === '' ? containerRoot : posix.join(containerRoot, rel);
}
export function stagingPathToContainer(stagingRoot, stagingPath, containerRoot = '/workspace') {
    const absolute = resolve(stagingPath);
    if (!isInsideHost(stagingRoot, absolute))
        return undefined;
    const rel = relative(resolve(stagingRoot), absolute).split(sep).join('/');
    return rel === '' ? containerRoot : posix.join(containerRoot, rel);
}
export function containerPathToStaging(stagingRoot, containerPath, containerRoot = '/workspace') {
...
export function translateInputPath(input, cwd, sourceRoot, stagingRoot, containerRoot = '/workspace') {
    if (posix.isAbsolute(input) && (input === containerRoot || input.startsWith(`${containerRoot}/`))) {
        ...
        if (mapped === undefined)
            throw new Error(`path escapes container workspace: ${input}`);
        return mapped;
    }
